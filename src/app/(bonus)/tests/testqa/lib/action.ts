...
export async function submitTestForm(prevState: TestFormState, formData: FormData): Promise<TestFormState> {
	...
	const rawTotalPoints = formData.get('totalPoints') as string | null;
	...
	// Получаем и преобразуем totalPoints
	const totalPoints = rawTotalPoints ? Number(rawTotalPoints) : 0;
	if (isNaN(totalPoints)) {
		console.warn('totalPoints не является числом:', rawTotalPoints);
	}
	...
	console.log('Тест пройден:', {
		...
		Результат: totalPoints + ' баллов из 36',
		Тест: 'Итоговый тест: Тестирование ПО',
	});
